...
workspace_dir = "/ghome/Alamgir/mPLUG"
...
def train_scst(model, data_loader, test_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config, do_amp=False,
    ...
    for i, (image, caption, object_labels, image_ids, gold_caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        caption = tokenizer(caption, padding='longest', truncation=True, max_length=args.max_input_length, return_tensors="pt", clean_up_tokenization_spaces=False).to(device)
        question_input = tokenizer(question_input, padding='longest', truncation=True, max_length=args.max_input_length, return_tensors="pt", clean_up_tokenization_spaces=False).to(device)
...
def evaluate(model, data_loader, dataset, tokenizer, device, config):
    ...
    for n, (image, caption, image_ids, gold_caption) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):        
        ...
        #{'Bleu_??4': 0.9999999999863945, 'Bleu_2': 0.9999999999859791, 'Bleu_3': 0.9999999999854866, 'Bleu_4': 0.999999999984889, 'METEOR': 1.0, 'ROUGE_L': 1.0, 'CIDEr': 2.7246232035629268, 'SPICE': 0.40389416048620613}
...
def main(args, config):
    # Set offline mode to avoid internet issues
    os.environ['TRANSFORMERS_OFFLINE'] = '1'
    os.environ['HF_DATASETS_OFFLINE'] = '1'
    os.environ['HF_HUB_OFFLINE'] = '1'
    
    # ===== CONFIG FILE FIX =====
    # Set the correct full paths for ghome directory
    config['bert_config'] = '/ghome/Alamgir/mPLUG/configs/config_bert_stride3.json'
    config['text_encoder'] = "/ghome/Alamgir/mPLUG/bert-base-uncased"
    config['text_decoder'] = "/ghome/Alamgir/mPLUG/bert-base-uncased"
    
    # Verify the config file exists
    if not os.path.exists(config['bert_config']):
        raise FileNotFoundError(f"BERT config file not found: {config['bert_config']}")
    # ===========================
    ...
    local_tokenizer_path = "/ghome/Alamgir/mPLUG/bert-base-uncased"
    ...
    if args.checkpoint:
        # Try to load with weights_only=True first, fall back to False if it fails
        try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/ghome/Alamgir/mPLUG/configs/VQA.yaml')
    parser.add_argument('--checkpoint', default='')
    parser.add_argument('--output_dir', default='/ghome/Alamgir/mPLUG/output/vqa')
    ...
    parser.add_argument('--distributed', action='store_true')  # Fixed: use action instead of type=bool
    ...
    # Add local_rank for torchrun compatibility
    parser.add_argument('--local_rank', type=int, default=-1, help='local rank for distributed training')
    
    args = parser.parse_args()

    # Handle local_rank from environment if not provided
    if args.local_rank == -1 and 'LOCAL_RANK' in os.environ:
        args.local_rank = int(os.environ['LOCAL_RANK'])

    # Load config using the same approach as your working script
    try:
        from ruamel.yaml import YAML
        yaml = YAML(typ='rt')
    ...
    # Save config with proper YAML handling
    try:
